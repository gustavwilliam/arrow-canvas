...
const selectLine = (point) => {
  ...
  if (selectionStore.point && selectionStore.point.equals(point)) {
    ...
    const currentIndex = possibleSelections.findIndex(
      (line) => line === selectionStore.line
    );
...
function handleMouseOver(point) {
  ...
  if (
    lastLine &&
    lastLine.points.length > 0 &&
    lastLine.points[lastLine.points.length - 1].equals(point)
  ) {
  ...
  if (
    lastLine &&
    lastLine.points.length > 0 &&
    !lastLine.points[lastLine.points.length - 1].isAdjacent(point)
  ) {
