<template>
  <div
    :ref="refId"
    class="inline-block align-bottom w-full aspect-square relative"
  ...
  >
    <div v-if="intersectingLines.length > 0">
      ...
      >
        <div class="relative">
          ...
          <img
            :class="
              'w-full h-full absolute inset-0 ' + point.connectorTransform()
            "
            draggable="false"
            :src="point.connectorSrc()"
            v-if="point.hasConnector()"
            alt="Line connector"
          />
        </div>
      </div>
    ...
    <div
      class="absolute -inset-1 rounded-full z-40"
...
const refId = computed(
  () => `square-${positionPoint.value.x}-${positionPoint.value.y}`
);
